...
function updateTotals() {
  ...
  totalBenefitsSpan.textContent = totalBenefits.toFixed(2);
...
function renderCashflowChart(benefits, costs, discountRate) {
  ...
  cashflowChart = new Chart(ctx, {
    ...
    options: {
      ...
      scales: {
        y: {
          title: { display: true, text: 'Yearly Amount (USD)' },
        beginAtZero: true,
...
function buildSeriesAndMetrics() {
  ...
  if (chartNote) {
    chartNote.textContent = `Note: Initial investment of $${upfrontInvestment.toFixed(2)} occurs at year 0 and is not shown in the bars below.`;
  }
  ...
  // Totals for header (Years 1..N only for the little blue strip above buttons)
  let sumBenefits = 0, sumCosts = 0;
  for (let i = 1; i <= projectLifetime; i++) {
    sumBenefits += benefits[i];
    sumCosts += costs[i];
  }
  totalBenefitsSpan.textContent = sumBenefits.toFixed(2);
  totalCostsSpan.textContent    = sumCosts.toFixed(2);
 ...
 *******************************/
const OPENAI_API_KEY = "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"; // move server-side in production

async function fetchNarrativesEasyEnglish(M, chartPNG) {
  const yearlyLines = M.benefits.map((b, i) => {
    const label = (i === 0) ? "Initial Investement" : `Year ${i}`;
    const bv = (b || 0).toFixed(2);
    const cv = (M.costs[i] || 0).toFixed(2);
    return `${label}: Benefits $${bv}, Costs $${cv}`;
  }).join('\n');

  const prompt = `
    Please return ONLY a compact JSON object with three keys:
    - "overview": string
    - "analysis": array of 6 numbered lists strings
    - "chartExplanation": string   // one friendly paragraph in 5-6 farmers friendly easy sentences that explains the attached chart including benefits, costs bars, cumulative line, undiscounted payback and discounted payback.
    - "conclusion": string
    No code fences. No extra text.

    Write in VERY EASY English for farmers. Use friendly, short sentences and simple words.

    Make the "analysis" number lists MORE DESCRIPTIVE in VERY EASY English for farmers:
    - Exactly six number lists (1 to 6) in this order: Return on Investemnt (ROI), Undiscounted Payback Period, Discounted Payback Period, Net Present Value (NPV), Internal Rate of Return (IRR), Benefit-Cost Ratio (BCR).
    - Each lists must use this HTML structure:
      <strong class="metric-name" style="color:blue;">METRIC</strong> — <em class="explanation">4–5 short sentences in easy English. Explain what the number means, why it matters for the farm, and any caution if needed.</em>

    In the "overview", include a short summary of the numbers (Return on Investemnt (ROI), Undiscounted Payback Period, Discounted Payback Period, Net Present Value (NPV), Internal Rate of Return (IRR), Benefit-Cost Ratio (BCR)) in simple words in a descriptive way in 5-6 simple sentences.

    Context for the report:
    - Initial Investment (year 0): $${M.upfrontInvestment.toFixed(2)}
    - Project years: ${M.projectLifetime}
    - Discount Rate (%): ${M.discountRate}
    - Our numbers:
      ROI: ${M.roi.toFixed(2)}%
      Undiscounted Payback (years from year 0): ${isFinite(M.payback) ? M.payback.toFixed(2) : 'N/A'}
      Discounted Payback: ${isFinite(M.discountedPayback) ? M.discountedPayback.toFixed(2) : 'N/A'}
      NPV ($): ${M.npv.toFixed(2)}
      IRR (%): ${M.irr === null ? 'N/A' : M.irr.toFixed(2)}
      BCR: ${M.bcr.toFixed(2)}
    - Year by year money (include Initial Investement):
    ${yearlyLines}

    Please remind that cash comes at the end of each year (except the Initial Investement at time 0).`
  ;
  ...
  const body = {
      ...
      // Attach the chart image if we have it
        ...(chartPNG ? [{
          role: "user",
          content: [
  ...
  throw new Error("Unexpected AI narrative format.");
}
...
async function generateAndShowReport() {
  ...
  try {
    ...
    const html = `
      ...
      <h2>Analysis</h2>
      <ol>${(currentNarratives.analysis||[]).map(x=>`<li>${x}</li>`).join('')}</ol>
      ...
      content: [
        ...
        ...(chartPNG ? [{
          ...
          stack: [
            ...
            { text: `Note: Initial investment of $${fmt2(M.upfrontInvestment)} happens at Year 0 and is not shown in the bars above.`, style: 'note' }
          ],
          margin: [0, 0, 0, 8]
        }] : []),
